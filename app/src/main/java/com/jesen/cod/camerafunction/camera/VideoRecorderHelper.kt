...
class VideoRecorderHelper(var context: Activity, private var mCamera: Camera,
                          private var rotation: Int, private var surface: Surface) {
    var mMediaRecorder: MediaRecorder? = null
    var isRunning = false
    private val filePath = FileUtil.createVideoFile(context)?.absolutePath
    ...
    fun startRecord() {
        try {
            ...
            mMediaRecorder?.let {
                ...
                //it.setVideoSize(320, 480)// 设置视频大小
                it.setVideoFrameRate(25)  // 设置帧率
                it.setMaxDuration(40000) //设置最大录像时间为10s
                ...
                it.start()
                isRunning = true
                Outil.log("start recording video, path is :$filePath")
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    fun stopRecord() {
        mMediaRecorder?.stop()
        isRunning = false
        Outil.log("video recording is Stop.")
        Toast.makeText(context, "视频保存路径：$filePath", Toast.LENGTH_LONG).show()
    }
